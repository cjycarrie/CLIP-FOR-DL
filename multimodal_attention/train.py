...
def multilabel_asymmetric_loss(logits, targets, gamma_pos=0, gamma_neg=4, clip=0.05, eps=1e-8, reduction='mean'):
    """
    多标签Asymmetric Loss
    Args:
        logits: [batch_size, num_classes]，未经过sigmoid
        targets: [batch_size, num_classes]，0/1标签
        gamma_pos: 正样本focusing参数
        gamma_neg: 负样本focusing参数
        clip: 负样本概率上限，防止负样本主导
        eps: 防止log(0)
    ...
    if clip is not None and clip > 0:
        probs_neg = (probs_neg + clip).clamp(max=1)

    pos_loss = targets * torch.log(probs_pos.clamp(min=eps))
    neg_loss = (1 - targets) * torch.log(probs_neg.clamp(min=eps))

    if gamma_pos > 0:
        pos_loss *= (1 - probs_pos) ** gamma_pos
    if gamma_neg > 0:
        neg_loss *= probs_pos ** gamma_neg
        ...
        try:
            ...
            if DEVICE.type == 'mps':
                try:
                    ...
                    # 应用多模态注意力
                    enhanced_features, attn_weights = models['multimodal_attention'](
                        image_features,
                        text_features
                    )
            ...
            # 计算logits
            logits = enhanced_features @ text_features.T  # [batch_size, num_classes]
            # 保证labels为float
            labels = labels.float()
            # 计算asymmetric loss
            loss = multilabel_asymmetric_loss(logits, labels)
            
            # 计算预测结果
            with torch.no_grad():
                similarities = (enhanced_features @ text_features.T) / MODEL_CONFIG['temperature']
...
def initialize_models(device):
    ...
    # 初始化多模态注意力模块
    multimodal_attention = MultiModalAttention().to(device)
...
class MultiModalAttention(nn.Module):
    def __init__(self):
        super().__init__()
        # 图像特征投影
        self.image_proj = nn.Linear(MODEL_CONFIG['shared_embedding_size'], MODEL_CONFIG['shared_embedding_size'])
        # 文本特征投影
        self.text_proj = nn.Linear(MODEL_CONFIG['shared_embedding_size'], MODEL_CONFIG['shared_embedding_size'])
        # 注意力计算
        self.attention = nn.Linear(MODEL_CONFIG['shared_embedding_size'], 1)
        # 输出投影
        self.output_proj = nn.Linear(MODEL_CONFIG['shared_embedding_size'], MODEL_CONFIG['shared_embedding_size'])
    ...
    def forward(self, image_features, text_features):
        ...
        Returns:
            enhanced_features: [batch_size, embedding_dim]
            attn_weights: [batch_size, num_classes]
        """
        # 投影特征
        image_proj = self.image_proj(image_features)  # [batch_size, embedding_dim]
        text_proj = self.text_proj(text_features)     # [num_classes, embedding_dim]
        
        # 计算注意力分数
        # 扩展文本特征以匹配批次大小
        text_proj_expanded = text_proj.unsqueeze(0).expand(image_proj.size(0), -1, -1)  # [batch_size, num_classes, embedding_dim]
        image_proj_expanded = image_proj.unsqueeze(1)  # [batch_size, 1, embedding_dim]
        
        # 计算注意力权重
        attn_scores = self.attention(torch.tanh(image_proj_expanded + text_proj_expanded))  # [batch_size, num_classes, 1]
        attn_weights = F.softmax(attn_scores.squeeze(-1), dim=1)  # [batch_size, num_classes]
        
        # 应用注意力权重
        attended_features = torch.bmm(attn_weights.unsqueeze(1), text_proj_expanded).squeeze(1)  # [batch_size, embedding_dim]
        
        # 特征融合
        enhanced_features = self.output_proj(image_proj + attended_features)
        
        return enhanced_features, attn_weights

def save_attention_visualization(images, attn_weights, disease_list, batch_idx, epoch):
    """保存注意力权重可视化
    
    Args:
        images: 原始图像 [B, V, C, H, W]
        attn_weights: 注意力权重 [B, num_heads, num_diseases]
        disease_list: 疾病列表
        batch_idx: 批次索引
        epoch: 当前epoch
    """
    # 创建保存目录
    os.makedirs('logs/attention', exist_ok=True)
    
    # 处理每个样本
    for i in range(images.size(0)):
        # 获取当前样本的注意力权重
        sample_weights = attn_weights[i]  # [num_heads, num_diseases]
        
        # 对每个视角创建可视化
        for v in range(images.size(1)):
            # 获取原始图像
            img = images[i, v].cpu().numpy()
            ...
            # 绘制注意力权重
            plt.subplot(1, 2, 2)
            plt.imshow(sample_weights.cpu().numpy(), cmap='viridis')
            plt.colorbar()
            plt.xticks(range(len(disease_list)), disease_list, rotation=45, ha='right')
            plt.yticks(range(sample_weights.size(0)), [f'Head {h}' for h in range(sample_weights.size(0))])
            plt.title('Attention Weights')
            
            # 保存图像
            plt.tight_layout()
            plt.savefig(f'logs/attention/epoch{epoch}_batch{batch_idx}_sample{i}_view{v}.png')
            plt.close()
            
            # 保存注意力权重数据
            np.save(
                f'logs/attention/epoch{epoch}_batch{batch_idx}_sample{i}_view{v}_weights.npy',
