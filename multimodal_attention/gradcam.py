...
class GradCAM:
    ...
    def generate_cam(self, input_tensor, target_category=None):
        ...
        target = torch.zeros_like(model_output, device=self.device)
...
def main(args):
    ...
    for i, (images, labels, findings, view_types) in enumerate(val_loader):
        if i == args.sample_index:
            # 获取两个视图
            frontal_image = images[0, 0].to(device)  # [3, 224, 224]
            lateral_image = images[0, 1].to(device)  # [3, 224, 224]
    ...
    for disease in positive_diseases:
        ...
        # 对两个视图分别生成GradCAM
        for view_idx, (view_name, image) in enumerate([('Frontal', frontal_image), ('Lateral', lateral_image)]):
            # 生成CAM
            cam = grad_cam.generate_cam(image.unsqueeze(0), target_category=None)
            
            # 保存结果
            save_path = os.path.join('logs/gradcam', f'sample_{args.sample_index}_{disease}_{view_name}.png')
            show_cam_on_image(
                image.cpu().numpy().transpose(1, 2, 0),
                cam,
                f"{disease} - {view_name} View",
                save_path
            )

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate GradCAM visualizations')
    parser.add_argument('--sample_index', type=int, default=0, help='Index of the validation sample to visualize')
