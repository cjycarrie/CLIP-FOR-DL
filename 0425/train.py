...
def train_epoch(models, train_loader, optimizer, epoch, disease_list):
    ...
    for i, (images, labels, findings, view_types) in enumerate(train_loader):
        try:
            ...
            # 将数据移到设备上
            images = images.to(DEVICE)  # [batch_size, 2, 3, 224, 224]
            labels = labels.to(DEVICE)  # [batch_size, num_classes]
            ...
            for v in range(views):
                view_images = images[:, v]  # [batch_size, 3, 224, 224]
            ...
            # 获取文本特征
            text_features = get_text_features(disease_list, models['tokenizer'], 
                                            models['text_model'], models['text_projector'])
...
def get_text_features_with_findings(diseases, tokenizer, text_model, text_projector, enhanced_prompts, device=DEVICE):
    """获取疾病文本的特征表示，包括增强的提示"""
    prompts = [f"{enhanced_prompt} {disease}." for disease, enhanced_prompt in zip(diseases, enhanced_prompts)]
